...
public class MyBuildForest{
  
  private static final Logger log = LoggerFactory.getLogger(MyBuildForest.class);
  ...
  private void buildForest() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    MyTreeBuilder forestBuilder;
    
//    Dataset dataset,int numTrees,String i_data
    
    
    forestBuilder = new MyTreeBuilder(seed,treeBuilder, dataPath, datasetPath, nbTrees);
    
//    if (isPartial) {
//      log.info("Partial Mapred implementation");
//      forestBuilder = new forestBuilder(treeBuilder, dataPath, datasetPath, seed, getConf());
//    } else {
...
//    int splitNum = 1;
    MyDecisionForest forest = forestBuilder.buildTree(outputPath);
    
    time = System.currentTimeMillis() - time;
    log.info("Build Time: {}", MyDFUtils.elapsedTime(time));
...
//  protected static Data loadData(Dataset dataset) throws IOException {
