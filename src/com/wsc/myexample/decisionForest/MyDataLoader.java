 ...
 */
public final class MyDataLoader {
  
  private static final Logger log = LoggerFactory.getLogger(MyDataLoader.class);
...
//    Preconditions.checkArgument(tokens.length == attrs.length, "Wrong number of attributes in the string");
    ...
    if(tokens.length != attrs.length){
//    	System.out.println("--attrs长度："+ attrs.length + "--tokens长度："+ tokens.length + "	：Wrong number of attributes in the string");
    	return false;
    }
   ...
   *          data path
   */
  public static MyDataset generateDataset(CharSequence descriptor,
                                        boolean regression,
                                        String path) throws DescriptorException, IOException {
