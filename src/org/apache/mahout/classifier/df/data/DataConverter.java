...
//    	        "Wrong number of attributes in the string");
    ...
    for (int attr = 0; attr < nball; attr++) {
      ...
      if (dataset.isNumerical(aId)) {
    	  if("".equals(token) || "IPHONE".equals(token) || "www.10010.com".equals(token)) token = "0.0";
    	  double value;
        try {
...
//			e.printStackTrace();
			value = 0.0;
//			vector.set(aId++, Double.parseDouble("0.0"));
		}
        
        vector.set(aId++, value);
      } else { // CATEGORICAL
