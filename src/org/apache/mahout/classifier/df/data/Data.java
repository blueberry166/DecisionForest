...
public class Data implements Cloneable {
	 ...
	 */
	public Data bagging(Random rng, boolean[] sampled, int sampleN,List<Instance> test) throws CloneNotSupportedException {
		int datasize = size();
		...
		for (int i = 0; i < datasize; i++) {
			if (!sampled[i]) {
				test.add(instances.get(i).clone());
			}
		}
	...
	public double[] values(int attr) {
		...
		for (Instance instance : instances) {
			if (instance == null) {
				...
				System.out.println(instance.get(attr));
			}
	 ...
	 * This method can be used when the criterion variable is the categorical
	 * attribute.
	 * 
